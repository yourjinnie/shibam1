import React from "react";
import SliderMe from "../components/SliderMe";
import ShareIcons from "../components/ShareIcons";
import { Link } from "react-router-dom";

const Inner8 = () => {
  return (
    <>
      <section className="CaseInnerWrapper">
        <div className="CaseInnerContainer">
          <section className="InnerHero">
            <img
              src="/assets/15-2479152f.png"
              alt="Periodical Health Assessment of"
              style={{ width: "100%" }}
            />
            <h2 className="InnerHeroH1 font-face-futur-md-bt">
              Periodical Health Assessment of{" "}
              <span className="orange"> Storage Tanks.</span>{" "}
            </h2>
            <div className="tagsD font-face-futur-HV" id="topGap">
            <Link to="/sectors" className="tags2">Others</Link>
            <Link to="/services/dimension-control" className="tags2">Dimension Control</Link>
            </div>
            <div className="bar">
              <h2 className="InnerHeroH2 font-face-futur-md-bt">
                <div className="lineabc" />
                The Problem at Hand
              </h2>
              <p className="font-face-futur-BK innerPara">
                The challenge involved conducting geometrical measurements and
                inspections on diverse storage tanks, including external
                floating and fixed roof tanks, to assess dimensional compliance
                with API 653 standards.
              </p>
            </div>
            <div className="why font-face-futur-md-bt">
              <h1>
                Why <span>SIXD?</span>
              </h1>
              <p className="whyPara font-face-futur-LT">
                Equipped with intrinsic and flame proof-certified systems,
                experts with international exposure work in offshore conditions
                and have experience with complete offshore platforms as built
                through survey and modelling with required deliveries
                internationally.
              </p>
            </div>
            <div className="section-inner">
              <div className="lineabc2" />
              <h1 className="section-heading-inner font-face-futur-BK">
                Scope of Work
              </h1>
              <div className="card-container-inner">
                <p className="font-face-futur-LT" />
                <div id="topGap2">
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        Conduct precise 3D measurements, alignment checks, and
                        accurate positioning for equipment, machineries, and
                        assemblies.
                        <br /> <br />
                        Provide expert assistance in levelling and installation
                        processes to ensure optimal functionality.
                      </p>
                    </div>
                  </div>
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        Perform thorough dimensional inspections covering
                        out-of-roundness, verticality, base settlement, and
                        distortions.
                        <br /> <br />
                        Conduct roof analysis and critical section thickness
                        analysis to assess structural integrity.
                      </p>
                    </div>
                  </div>
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        Evaluate shell settlement, analyze tank floor plate
                        conditions, and inspect top plate edge settlement and
                        tilt.
                        <br /> <br />
                        Examine floating roof rim space and gap, conducting
                        structural analysis to identify potential issues.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="howwas">
              <div className="how-cnt">
                <span className="deco2" />
                <h1 className="font-face-futur-BK howh1">
                  How was the problem tackled
                </h1>
                <ul className="font-face-futur-LT howp">
                  <li>
                    Data evaluation was conducted in strict adherence to API
                    Standard 653, employing PTB Certified Mathematical
                    Algorithm-based Dimension analysis software.
                    <br /> <br />
                    This software facilitated a meticulous examination of
                    collected data, enabling the creation of comprehensive
                    deviation reports at various vertical and horizontal
                    sections.
                    <br /> <br />
                    The utilization of certified algorithms ensured precision in
                    the analysis, allowing for accurate assessments of
                    out-of-roundness, verticality, settlement, distortions, and
                    other critical factors.
                    <br /> <br />
                    This method not only aligned with industry best practices
                    but also provided a systematic and reliable means to
                    evaluate and address geometrical complexities in storage
                    tanks.
                  </li>
                </ul>
              </div>
              <div className="how-img">
                <img src="/assets/t15-a54ddf9e.png" />
              </div>
            </div>
            <div className="page">
              <img src="/assets/p15-652ac3e4.png" className="page" />
            </div>
            <div className="arch">
              <div className="fxm">
                <span className="deco3" />
                <h1 className="font-face-futur-md-bt">
                  The <span className="orange">Difference</span> Achieved
                </h1>
              </div>
              <div className="side-data">
                <div className="side-image">
                  <img src="/assets/i1-a28f622a.gif" alt="Animated Frame" />
                </div>
                <div className="side-col">
                  <div className="row-side">
                    <div className="card-main-side">
                      {" "}
                      <span className="deco4" />
                      <h1 className="font-face-futur-BK">Enhanced Accuracy</h1>
                      <p className="font-face-futur-LT">
                        Reliable dimensional analysis and data evaluation
                        according to API Standard 653, increased the overall
                        accuracy of the process.
                      </p>
                    </div>
                    <div className="card-main-side">
                      {" "}
                      <span className="deco4" />
                      <h1 className="font-face-futur-BK">Cost Saving</h1>
                      <p className="font-face-futur-LT">
                        Implementing Certified Dimension analysis software
                        minimized errors, leading to efficient inspection
                        processes and cost savings in terms of maintenance,
                        repair, and potential downtime.
                      </p>
                    </div>
                  </div>
                  <div className="row-side">
                    <div className="card-main-side">
                      {" "}
                      <span className="deco4" />
                      <h1 className="font-face-futur-BK">Precision</h1>
                      <p className="font-face-futur-LT">
                        The solution allowed precise examination of critical
                        factors like settlement, distortions, and thickness
                        analysis, contributing to informed decision-making and
                        improved structural integrity.
                      </p>
                    </div>
                    <div className="card-main-side">
                      {" "}
                      <h1 className="font-face-futur-BK" />
                      <p className="font-face-futur-LT" />
                    </div>
                  </div>
                  <p className="p-side font-face-futur-LT">
                    Our end-to-end solutions have not only effectively tackled
                    the complexities of Green Hydrogen projects but have also
                    led to significant advancements in precision, cost
                    optimization, environmental compliance, and seamless project
                    transition for ongoing operational success.
                  </p>
                </div>
              </div>
            </div>
            <div className="sharemy">
              <div className="sharex">
                <h1 className="font-face-futur-md-bt">Share this Case Study</h1>
                <div className="shareb">
                  <div className="share-50">
                    <div className="shareby font-face-futur-md-bt">
                      Share via
                    </div>
                    <ShareIcons />
                  </div>
                </div>
              </div>
            </div>
            <div className="innnercards">
              <h1 className="font-face-futur-BK">View more</h1>
              <div className="wrappermax">
                <Link to="/inner/13">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Physical Parts to Manufacturing Blueprints for Essential
                      Spares and Components.{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Others</p>
                      <p>Re-Engineering &amp; Supply</p>
                    </div>
                    <img src="/assets/id8c1-76b4db6d.png" />
                  </div>
                </Link>
                <Link to="/inner/1">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Alignment Inspection and Structural Integrity Solutions
                      for Overhead Cranes{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Others</p>
                      <p>Dimension Control</p>
                    </div>
                    <img src="/assets/id3c2-dadc1ecf.png" />
                  </div>
                </Link>
                <Link to="/inner/15">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Enhancing Passenger Comfort with Wipro Limited{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Others</p>
                      <p>Re-engineering &amp; Supplyy</p>
                    </div>
                    <img src="/assets/id1c3-1961532f.png" />
                  </div>
                </Link>
              </div>
            </div>
          </section>
        </div>
      </section>
      <SliderMe />
    </>
  );
};

export default Inner8;

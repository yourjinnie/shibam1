import React from "react";
import SliderMe from "../components/SliderMe";
import ShareIcons from "../components/ShareIcons";
import { Link } from "react-router-dom";

const Inner3 = () => {
  return (
    <div>
      &lt;&gt;
      <section className="CaseInnerWrapper">
        <div className="CaseInnerContainer">
          <section className="InnerHero">
            <img
              src="/assets/5-9851b438.png"
              alt="Precise Revamping and Modernization for"
              style={{ width: "100%" }}
            />
            <h2 className="InnerHeroH1 font-face-futur-md-bt">
              Precise Revamping and Modernization for{" "}
              <span className="orange">Offshore Platforms</span>{" "}
            </h2>
            <div className="tagsD font-face-futur-HV" id="topGap">
            <Link to="/sectors/oil-gas" className="tags2">Oil & Gas</Link>
            <Link to="/services/as-built" className="tags2">As-Built Engineering</Link>
            </div>
            <div className="bar">
              <h2 className="InnerHeroH2 font-face-futur-md-bt">
                <div className="lineabc" />
                The Problem at Hand
              </h2>
              <p className="font-face-futur-BK innerPara">
                Reliance Industries Limited (RIL) faced a daunting challenge in
                replacing six existing Coke Drums within a complex structure and
                platform arrangement at their facility in Jamnagar, Gujarat. The
                critical issue was determining whether the surrounding structure
                could be safely cut at a specific level to facilitate the
                removal and replacement of the old drums with new ones.
              </p>
            </div>
            <div className="why font-face-futur-md-bt">
              <h1>
                Why <span>SIXD?</span>
              </h1>
              <p className="whyPara font-face-futur-LT">
                Achieving precision in this operation, especially at an
                elevation of 106 meters, was paramount to avoid misalignment and
                costly errors. Our expertise in scanning complex industrial
                environments was critical to this project.
              </p>
            </div>
            <div className="section-inner">
              <div className="lineabc2" />
              <h1 className="section-heading-inner font-face-futur-BK">
                Scope of Work
              </h1>
              <div className="card-container-inner">
                <p className="font-face-futur-LT" />
                <div id="topGap2">
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        The project involved scanning six Coke Drums and all
                        associated structures, including platforms and supports.
                        The goal was to capture the entire geometry of the site
                        accurately.
                      </p>
                    </div>
                  </div>
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        Creating an exact geometric representation of the plant
                        in its existing condition (As-is model).
                      </p>
                    </div>
                  </div>
                  <div className="card-inner">
                    <div className="card-content-inner">
                      <img
                        src="data:image/png;base64,"
                        alt="data:image/png;base64,"
                        className="card-icon-inner"
                      />
                      <p className="card-paragraph-inner font-face-futur-LT innP">
                        Converting primitive model into PDS-compatible data,
                        ensuring seamless integration.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="howwas">
              <div className="how-cnt">
                <span className="deco2" />
                <h1 className="font-face-futur-BK howh1">
                  How was the problem tackled
                </h1>
                <ul className="font-face-futur-LT howp">
                  <li>
                    Our team executed the project with meticulous precision and
                    attention to detail.{" "}
                  </li>
                  <li>
                    {" "}
                    <br /> We employed{" "}
                    <b>advanced 3D laser scanning technology</b> to capture the
                    complex geometry of the Coke Drum unit and surrounding
                    structures at an elevation of 106 meters. This
                    high-elevation scanning required careful planning and
                    execution to ensure the desired level of accuracy.{" "}
                  </li>
                  <li>
                    <br /> Once the scan data was collected, our experts used
                    Cyclone software to create a primitive model that faithfully
                    represented the existing plant conditions.
                  </li>
                  <li>
                    <br /> This model was refined to meet RIL's specific
                    software compatibility requirements, ensuring a smooth
                    transition for further analysis and planning.
                  </li>
                </ul>
              </div>
              <div className="how-img">
                <img src="/assets/t5-f4085af0.png" />
              </div>
            </div>
            <div className="page">
              <img src="/assets/p5-e34ec461.png" className="page" />
            </div>
            <div className="arch">
              <div className="fxm">
                <span className="deco3" />
                <h1 className="font-face-futur-md-bt">
                  The <span className="orange">Difference</span> Achieved
                </h1>
              </div>
              <div className="side-data">
                <div className="side-image">
                  <img src="/assets/i1-a28f622a.gif" alt="Animated Frame" />
                </div>
                <div className="side-col">
                  <div className="row-side">
                    <div className="card-main-side">
                      {" "}
                      <span className="deco4" />
                      <h1 className="font-face-futur-BK">
                        Precision and Accuracy
                      </h1>
                      <p className="font-face-futur-LT">
                        Our expertise in high-elevation scanning and 3D
                        modelling ensured that RIL received precise geometric
                        data, enabling them to make informed decisions regarding
                        the structure's cutting and replacement.
                      </p>
                    </div>
                    <div className="card-main-side">
                      {" "}
                      <span className="deco4" />
                      <h1 className="font-face-futur-BK">Risk Mitigation</h1>
                      <p className="font-face-futur-LT">
                        With SixD, RIL minimized the risk of misalignment or
                        errors during the replacement process. This not only
                        saved time but also reduced potential costly setbacks.
                      </p>
                    </div>
                  </div>
                  <div className="row-side">
                    <div className="card-main-side">
                      {" "}
                      <h1 className="font-face-futur-BK" />
                      <p className="font-face-futur-LT" />
                    </div>
                    <div className="card-main-side">
                      {" "}
                      <h1 className="font-face-futur-BK" />
                      <p className="font-face-futur-LT" />
                    </div>
                  </div>
                  <p className="p-side font-face-futur-LT">
                    {" "}
                    In conclusion, our innovative approach to 3D laser scanning
                    and modelling played a pivotal role in optimizing the Coke
                    Drum replacement project for Reliance Industries Limited.
                  </p>
                </div>
              </div>
            </div>
            <div className="sharemy">
              <div className="sharex">
                <h1 className="font-face-futur-md-bt">Share this Case Study</h1>
                <div className="shareb">
                  <div className="share-50">
                    <div className="shareby font-face-futur-md-bt">
                      Share via
                    </div>
                    <ShareIcons />
                  </div>
                </div>
              </div>
            </div>
            <div className="innnercards">
              <h1 className="font-face-futur-BK">View more</h1>
              <div className="wrappermax">
                <Link to="/inner/8">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Physical Parts to Manufacturing Blueprints for Essential
                      Spares and Components.{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Others</p>
                      <p>Re-Engineering &amp; Supply</p>
                    </div>
                    <img src="/assets/id3c1-65ad2531.png" />
                  </div>
                </Link>
                <Link to="/inner/1">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Revolutionising Measurement and Alignment Procedures with
                      Steel &nbsp;{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Steel</p>
                      <p>Dimension Control</p>
                    </div>
                    <img src="/assets/id3c2-dadc1ecf.png" />
                  </div>
                </Link>
                <Link to="/inner/4">
                  <div className="cardmax">
                    <h1 className="font-face-futur-md-bt">
                      {" "}
                      Streamlining Project Management for Green Hydrogen Plants{" "}
                    </h1>
                    <div className="tagsmax font-face-futur-md-bt">
                      <p>Power</p>
                      <p>Owner’s Engineering</p>
                    </div>
                    <img src="/assets/id3c3-775b0b1f.png" />
                  </div>
                </Link>
              </div>
            </div>
          </section>
        </div>
      </section>
      <SliderMe />
    </div>
  );
};

export default Inner3;
